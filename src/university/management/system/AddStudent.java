...
public class AddStudent extends JFrame implements ActionListener {
    ...
    public AddStudent() {
        ...
        String rollno = "1533" + (new Random().nextInt(9000) + 1000);
        ...
        tfclassX = addTextField(600, 300);

        addLabel("Class XII (%)", 50, 350);
        tfclassXII = addTextField(200, 350);

        addLabel("NIC Number", 400, 350);
        tfnic = addTextField(600, 350);
    ...
    private boolean validateFields() {
        ...
        if (tfclassX.getText().trim().isEmpty())
            return showError("Class X % is required.");
        if (tfclassXII.getText().trim().isEmpty())
            return showError("Class XII % is required.");
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == submit) {
            ...
            try {
                Conn con = new Conn();
                String query = "INSERT INTO student (name, fname, rollno, dob, address, phone, email, class_x, class_xii, nic, degree, campus) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                PreparedStatement pst = con.c.prepareStatement(query);

                pst.setString(1, tfname.getText());
                pst.setString(2, tffname.getText());
                pst.setString(3, labelrollno.getText());
                ...
                pst.setString(8, tfclassX.getText());
                pst.setString(9, tfclassXII.getText());
                pst.setString(10, tfnic.getText());
                pst.setString(11, (String) cbdegree.getSelectedItem());
                pst.setString(12, (String) cbcampus.getSelectedItem());

                pst.executeUpdate();

                String queryFee = "INSERT INTO collegefee (rollno, degree, campus, semester, total) VALUES (?, ?, ?, ?, ?)";
                PreparedStatement pstFee = con.c.prepareStatement(queryFee);
                pstFee.setString(1, labelrollno.getText());
                pstFee.setString(2, (String) cbdegree.getSelectedItem());
                pstFee.setString(3, (String) cbcampus.getSelectedItem());
                pstFee.setString(4, "Semester 1");
                pstFee.setString(5, "0");

                pstFee.executeUpdate();
